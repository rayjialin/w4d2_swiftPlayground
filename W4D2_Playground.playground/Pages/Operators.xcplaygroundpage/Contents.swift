 ...
 */
postfix operator %%%
postfix func %%% (intValue: Int) -> Double{
...
/*:
 ...
 For example, [1,2,3] + [3,4] = [4,6], . If the array count size are not the same, then return nil
 */
func + (leftArray: [Int], rightArray: [Int]) -> [Int]{
    var intArray = [Int]()
    if leftArray.count == rightArray.count{
        for index in 0..<leftArray.count{
            intArray.append(leftArray[index] + rightArray[index])
        }
    }
    return intArray
}

let leftArray = [2,4,2,5,2,7,5]
let rightArray = [1,2,3,4,5,6,5]
